...
(function(util, data, fn, global) {
	...
	function fetch(uri, callback) {
		if (fetchingMods[uri]) {
			...
			fetchingMods[uri] = util.getAsset(getUrl(uri), cb,
					data.config.charset);
...
(function(util, data, fn, global) {
	...
	if (loaderDir
			&& (global.location.search.indexOf('seajs-debug') !== -1 || document.cookie
					.indexOf('seajs=1') !== -1)) {
		config.debug = true;
...
window.define = function(define) {
	...
	return function(id) {
		if (arguments.length >= 3 && typeof id === 'string'
				&& define._____ids[id]) {
			return;
		}
...
}(define);/* |xGv00|cc41e58e14d5312cf1e7a57331f60d54 */